...
type Team = { id: string; team_name: string; captain_email: string; college: string; member_count: number; score: number; current_clue: number; passcode: string; created_at: string; };
type GameLog = { id: string; action_type: string; points_awarded: number; created_at: string; teams: { id: string; team_name: string }; clues: { id: number; title: string }; };
...
export default function AdminPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // 2. Fetch Logs
      const { data: logsData, error: logsError } = await supabase.from("game_logs").select(`id, action_type, points_awarded, created_at, teams ( id, team_name ), clues ( id, title )`).order("created_at", { ascending: false });
      if (logsError) throw logsError;
      if (logsData) setLogs(logsData as any);
      
      // 3. Fetch Game State (Reveal Status)
      const { data: stateData, error: stateError } = await supabase.from("game_state").select("is_leaderboard_revealed").eq("id", 1).single();
      if (!stateError && stateData) setIsRevealed(stateData.is_leaderboard_revealed);
  ...
  // --- NEW: TOGGLE LEADERBOARD REVEAL ---
  const toggleLeaderboardReveal = async () => {
    const action = isRevealed ? "HIDE" : "REVEAL";
    if (window.confirm(`Are you sure you want to ${action} the real Leaderboard to the public?`)) {
      const newState = !isRevealed;
      await supabase.from("game_state").update({ is_leaderboard_revealed: newState }).eq("id", 1);
      setIsRevealed(newState);
      alert(`Leaderboard is now ${newState ? 'REVEALED' : 'HIDDEN'}.`);
    }
  };

  const handleDeleteTeam = async (teamId: string, teamName: string) => {
    const isConfirmed = window.confirm(`⚠️ WARNING: Are you sure you want to delete the team "${teamName}"? This cannot be undone.`);
    if (isConfirmed) {
      setIsLoading(true);
      try {
  ...
  const calculateTimeTaken = (team: Team) => {
    ...
    if (hasFinished) {
      ...
      if (teamLogs.length > 0) endTime = new Date(teamLogs[0].created_at).getTime();
    }
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[url('/BG.png')] bg-cover bg-center">
          ...
          <div className="relative z-10 w-full max-w-sm bg-black/80 backdrop-blur-md border border-red-900/50 rounded-lg p-8 shadow-[0_0_50px_rgba(255,0,0,0.1)] text-center">
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-6">
              <input type="password" value={passcodeInput} onChange={(e) => setPasscodeInput(e.target.value)} placeholder="ENTER OVERRIDE CODE" className="w-full bg-black/50 border-b border-red-900/50 pb-2 text-center text-red-400 font-mono tracking-[0.3em] text-sm focus:outline-none focus:border-red-500 transition-colors" />
              <button type="submit" className="w-full py-3 bg-red-900/20 hover:bg-red-900/40 border border-red-900/50 hover:border-red-500 text-red-500 text-xs tracking-[0.2em] uppercase transition-all">Authenticate</button>
            </form>
            <Link href="/" className="inline-block mt-8 text-[10px] text-white/30 hover:text-white/70 tracking-widest uppercase">&larr; Return to Public Sector</Link>
          </div>
        </div>
      ) : (
        <div className="flex flex-col min-h-screen">
          <header className="bg-black border-b border-[#D4AF37]/20 px-4 md:px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 sticky top-0 z-50">
            ...
            <div className="flex items-center gap-2 md:gap-4 text-[10px] md:text-xs font-bold tracking-widest uppercase overflow-x-auto w-full md:w-auto pb-2 md:pb-0 hide-scrollbar">
              <button onClick={() => setActiveTab("teams")} className={`px-4 py-2 transition-colors whitespace-nowrap rounded ${activeTab === "teams" ? "text-black bg-[#D4AF37]" : "text-[#D4AF37] hover:bg-[#D4AF37]/10"}`}>Teams Data</button>
              <button onClick={() => setActiveTab("logs")} className={`px-4 py-2 transition-colors whitespace-nowrap rounded ${activeTab === "logs" ? "text-black bg-[#D4AF37]" : "text-[#D4AF37] hover:bg-[#D4AF37]/10"}`}>Global Audit Logs</button>
              
              {/* THE NEW REVEAL TOGGLE BUTTON */}
              <button onClick={toggleLeaderboardReveal} className={`px-4 py-2 whitespace-nowrap rounded border transition-all ${isRevealed ? 'bg-red-900/40 text-red-400 border-red-500 hover:bg-red-900/60 shadow-[0_0_15px_rgba(255,0,0,0.3)]' : 'bg-green-900/30 text-green-400 border-green-500 hover:bg-green-900/50'}`}>
                {isRevealed ? '🔒 HIDE LEADERBOARD' : '📣 REVEAL WINNERS'}
              </button>

              <button onClick={fetchDashboardData} className="px-3 py-2 text-white/50 hover:text-white border border-white/20 hover:border-white/50 ml-auto md:ml-4 rounded whitespace-nowrap">↻ Refresh</button>
            </div>
          </header>

          <div className="flex-grow p-4 md:p-8 overflow-x-hidden">
            {isLoading && teams.length === 0 ? (
              <div className="flex justify-center items-center h-64 text-[#D4AF37] text-xs tracking-widest uppercase animate-pulse">Decrypting Database...</div>
            ) : (
                            ...
                            return (
                              <Fragment key={team.id}>
                                <tr className="border-b border-white/5 hover:bg-white/5 transition-colors text-xs text-white/80">
                                  ...
                                  <td className="p-4 text-center">{hasFinished ? <span className="text-green-400 font-bold tracking-widest text-[10px] uppercase">Finished</span> : team.current_clue}</td>
                                  <td className="p-4 text-center"><span className="px-2 py-1 bg-red-900/30 text-red-400 border border-red-900/50 font-mono tracking-widest rounded">{team.passcode}</span></td>
                                  <td className="p-4 text-right font-mono tracking-widest">
                                    <div className="flex flex-col items-end">
                                      ...
                                      <span className={`text-[9px] uppercase mt-1 ${hasFinished ? "text-green-400/50" : "text-yellow-500/50"}`}>{hasFinished ? "Final Time" : "Playing"}</span>
                                    </div>
                                  </td>
                                  <td className="p-4 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                      <button onClick={() => setExpandedTeamId(isExpanded ? null : team.id)} className="px-3 py-1.5 bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/30 rounded text-[10px] tracking-widest uppercase transition-colors">{isExpanded ? "Close" : "History"}</button>
                                      <button onClick={() => handleDeleteTeam(team.id, team.team_name)} className="px-3 py-1.5 bg-red-900/20 hover:bg-red-900/50 text-red-400 border border-red-900/30 hover:border-red-500 rounded text-[10px] tracking-widest uppercase transition-colors">Delete</button>
                                    </div>
                                  </td>
                                </tr>
                                {isExpanded && (
                                          ...
                                          <ul className="space-y-2">
                                            {teamLogs.map(log => (
                                              <li key={log.id} className="flex justify-between items-center text-[10px] md:text-xs font-mono border-b border-white/5 pb-2">
                                                ...
                                                <span className={`px-2 py-0.5 rounded uppercase tracking-widest ${log.action_type === 'HINT_USED' ? 'bg-yellow-900/30 text-yellow-500' : 'bg-green-900/30 text-green-500'}`}>{log.action_type}</span>
                          ...
                          {logs.map((log) => {
                            ...
                            return (
                              <tr key={log.id} className="border-b border-white/5 hover:bg-white/5 transition-colors text-xs text-white/80 font-mono">
                                <td className="p-4 text-white/40 whitespace-nowrap">{new Date(log.created_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</td>
                                <td className="p-4 font-sans font-bold text-white whitespace-nowrap">{log.teams?.team_name || "Unknown"}</td>
                                <td className="p-4"><span className={`px-2 py-1 rounded text-[10px] font-sans tracking-widest uppercase whitespace-nowrap ${isHint ? 'bg-yellow-900/30 text-yellow-500' : 'bg-green-900/30 text-green-500'}`}>{log.action_type}</span></td>
                                <td className="p-4 text-white/50">{log.clues?.title || "N/A"}</td>
                                <td className={`p-4 text-right font-bold ${isHint ? 'text-white/30' : 'text-[#D4AF37]'}`}>+{log.points_awarded}</td>
                              </tr>
                            )
                          })}
                        </tbody>
